package org.lw5hr.contest.db;

public class DatabaseConstants {
  public static String DEFAULT_LANG = "default_lang";
  public static String CURRENT_CONTEST = "current_contest";
  public static String NEW_CONTEST = "New Contest";
  public static String DXLOG_DB_PATH = "dxlog_db_path";
  public static String LIVE_CONTEST = "true";
  public static String LIVE_CONTEST_ON = "live_contest";
  public static String DEFAULT_LOCALE_LANGUAGE = "en";
  public static String DEFAULT_LOCALE_COUNTRY = "US";
  public static String NO_OPERATOR_SET = "NO OPERATOR SET";
  public static String DATE_FORMAT = "yyyy-MM-dd HH:mm:ss";

  public static String DX_LOG_CONTEST_NAME = "ContestName";

}
